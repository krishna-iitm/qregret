...
m = 1;             % Number of blocks for non-stationary rewards
...
[rewards,sample_mean_per_arm] = non_stat_channel_mult(N,T);

max_serv_rate = max(mean(sample_mean_per_arm, 2));

% simulate the arrival process (random for now)
A = 0+(2*(max_serv_rate-0.05))*rand(1, T);  % Arrival
% A =rand(1, T);
disp('Mean Arrival:');
display(mean(A));
...
plot_algorithm_queue_evolution(T, queue_evolution_algo, all_arms_queue_evolutions(optimal_arm,:), queue_evolution_qths,m, optimal_arm);

%plot cumulative service offered by each arm
plot_cumulative_service_offered(N, T, rewards, cum_ch_gain_algo(1,T), sum(channel_gains_qths));
