function [Q, arm_choices, channel_gains] = stahlbuhk_algo(N, T, rewards, A)
    ...
    while t <= T
        ...
        if current_queue == 0  % Empty period
            if is_first_empty_slot
                ...
                global_mu_hat(server) = total_mu(server)/t;
...
function [Q, armchoices, t] = learning_subroutine(t, N, T, rewards, A, current_queue)
    ...
    while current_queue > 0
        ...
        if is_exploration_time(n)
            ...
            mu_hat_p(selected_arm) = total_mu_p(selected_arm)/n;
